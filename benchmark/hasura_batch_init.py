...
class BatchInitPool(HasuraSubscriberPool):
...
async def run(args):
    ...
    try:
        ...
        try:
            ...
            print()
            print(
                "===== HASURA BATCH INIT CONNECTION FAILURE ====="
            )
        ...
        result = {
            ...
            "test":
                "subscription_batch_init",

            "purpose":
                (
                    "Multiplexing/batch scaling with a fixed "
                    "number of WebSocket clients and an increasing "
                    "number of structurally identical subscriptions."
                ),
            ...
            "configured_hasura_batch_size":
                args.hasura_batch_size,
            ...
            "concurrency_diagnostic": {
                ...
                "note":
                    (
                        ...
                        "required in this fixed-client batch test."
                    ),
            },
        ...
        if not valid_run:
            raise RuntimeError(
                "Batch-init run is methodologically invalid. "
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Hasura batch/multiplexing subscription-init benchmark "
            "with fixed WebSocket client count."
        )
    )
    ...
    if args.subscribers > args.topics:
        raise SystemExit(
            ...
            "because this benchmark assigns one unique "
            "topic to each subscription."
        )

    if args.hasura_batch_size <= 0:
        raise SystemExit(
            "--hasura-batch-size must be > 0."
        )

    if not args.barrier:
        raise SystemExit(
            "Batch-init measurements require --barrier."
        )
