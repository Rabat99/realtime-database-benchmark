"""
Kontrollierte Zero-Messserie.

query: Schreibrate konstant, Queryzahl variieren.
write: Queryzahl konstant, Schreibrate variieren.

Kein automatischer Latenz- oder SLA-Grenzwert. Apparaturfehler werden pro
geplanter Wiederholung begrenzt erneut ausgefuehrt und separat protokolliert.
Nur apparativ gueltige Laeufe gehen in die Stufenmediane ein.
"""
...
def remove_bench_containers() -> None:
    proc = subprocess.run(
        "docker ps -aq --filter name=zero-bench- | xargs -r docker rm -f",
        shell=True,
        capture_output=True,
        text=True,
    )
    if proc.returncode != 0:
        raise RuntimeError(
            "zero-bench-Container konnten nicht entfernt werden:\n"
            + proc.stderr.strip()
        )


def reset_cvr(wait_s: float) -> None:
    user = ENV.get("POSTGRES_USER")
    if not user:
        raise RuntimeError("POSTGRES_USER fehlt in .env")

    quoted_user = '"' + user.replace('"', '""') + '"'

    print("  zero-cache stoppen ...")
    subprocess.run(compose("stop", "zero-cache"), check=True, text=True)

    print("  zero_cvr neu anlegen ...")
    subprocess.run(
        compose(
            "exec",
            "-T",
            ...
            "ON_ERROR_STOP=1",
            "-c",
            "DROP DATABASE IF EXISTS zero_cvr WITH (FORCE)",
            "-c",
            f"CREATE DATABASE zero_cvr OWNER {quoted_user}",
        ),
        check=True,
        text=True,
    )

    print("  zero-cache starten ...")
    subprocess.run(compose("start", "zero-cache"), check=True, text=True)
    print(f"  warte {wait_s:.0f}s auf Zero ...")
    time.sleep(wait_s)
...
def run_preflight(args) -> None:
    cmd = [
        sys.executable,
        str(BENCH_DIR / "zero_preflight.py"),
        "--expect-messages",
    ...
    if proc.returncode != 0:
        raise RuntimeError(f"Zero-Preflight fehlgeschlagen: Returncode={proc.returncode}")


def prepare_run(args) -> float:
    remove_bench_containers()
    reset_cvr(args.restart_wait)
    run_preflight(args)

    size = cvr_mb()
    if size is None:
        raise RuntimeError("CVR-Groesse konnte nach Reset nicht bestimmt werden")
    print(f"  CVR-Ausgangszustand: {size:.1f} MiB")
    if size > args.reset_cvr_max_mb:
        raise RuntimeError(
            f"CVR nach Reset zu gross: {size:.1f} MiB > {args.reset_cvr_max_mb:.1f} MiB"
        )
    return size
...
def run_one(
    ...
    *,
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"zero_smoke.py fehlgeschlagen: Q={queries}, Rate={load_rate}, "
...
def stage_summary(stage: dict, runs: list[dict], requested_repeats: int) -> dict:
    ...
    realtime_cpu = [
        v
        for r in runs
        if (v := resource_value(r, "zero_path_cpu_mean_pct")) is not None
    ]
    repl_lag_max = [
        v
        for r in runs
        if (v := resource_value(r, "replication_lag_max_bytes")) is not None
    ]
    ...
    return {
        **stage,
        ...
        "replication_lag_max_median_bytes": median_or_none(repl_lag_max),
...
def make_run_record(item, result, *, pos, attempt, cvr_start_mb, apparatus_ok, bad):
    return {
        "position": pos,
...
def main() -> int:
    ...
    ap.add_argument("--reset-cvr-max-mb", type=float, default=20.0)
    ...
    if args.mode == "query":
        ...
        if len(set(query_values)) != len(query_values):
            raise RuntimeError("Query-Stufen enthalten Duplikate")
        stages = [
        ...
        if not rates or any(r <= 0 for r in rates):
            raise RuntimeError("write-Modus braucht --rates mit positiven Werten")
        if len(set(rates)) != len(rates):
            raise RuntimeError("Write-Stufen enthalten Duplikate")
        stages = [
            {
    ...
    summary = {
        ...
        "reset_cvr_max_mb": args.reset_cvr_max_mb,
    ...
    print(
        ...
        f"{'Delivery':>10} {'RT-CPU':>9} {'ReplLag':>12}"
    )
    ...
    for stage in sorted(summaries, key=lambda x: (x["queries"], x["load_rate"])):
        print(
            ...
            f"{fmt(stage['realtime_cpu_mean_median_pct'], 9, 1)} "
            f"{fmt(stage['replication_lag_max_median_bytes'], 12, 0)}"
        )
